...
(function ($, Drupal) {
  ...
  Drupal.behaviors.downloadCard = {
    attach: function (context, settings) {
      $('.download-card-wrapper').once().each(function () {
        const componentItem = $(this).get(0);
        componentItem.querySelector('.download-card').title =  `Baixar ${componentItem.querySelector('.download-card-title').innerText}`
      });
    }
  };
})(jQuery, Drupal);
